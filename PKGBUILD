...
package () {
    ...
    install -d -m644 "$pkgdir/etc/bspwm"
    install -d -m644 "$pkgdir/etc/sxhkd"
    install -m755 "$srcdir/$pkgname/MouseCenter" "$pkgdir/usr/bin/MouseCenter"
    	install -m644 "$srcdir/$pkgname/configs/bspwm/bspwmrc" "$pkgdir/etc/bspwm/bspwmrc"
    	install -m644 "$srcdir/$pkgname/configs/bspwm/autostart" "$pkgdir/etc/bspwm/autostart"
    	install -m644 "$srcdir/$pkgname/configs/bspwm/panel/conkyrc" "$pkgdir/etc/bspwm/conkyrc"
    	install -m644 "$srcdir/$pkgname/configs/bspwm/panel/conkyrc-laptop" "$pkgdir/etc/bspwm/conkyrc-laptop"
    	install -m644 "$srcdir/$pkgname/configs/bspwm/panel/conkyrc-desktop" "$pkgdir/etc/bspwm/conkyrc-desktop"
    	install -m644 "$srcdir/$pkgname/configs/sxhkd/sxhkdrc" "$pkgdir/etc/sxhkd/sxhkdrc"
